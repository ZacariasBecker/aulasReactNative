    ...
    return Array(rows).fill(0).map((_, row) => {
        return Array(columns).fill(0).map((_, column) => {
            return {
                ...
                column, // mesma coisa que column: column
                opened: false,
                flagged: false,
                mined: false,
                exploded: false,
...
const spreadMines = (board, minesAmount) => {
    ...
    while (minesPlanted < minesAmount) {
        const rowSel = parseInt(Math.random() * rows, 10) // 10 de base decimal
        const columnSel = parseInt(Math.random() * columns, 10) // 10 de base decimal
